...
Wow, how far we came! We can use Observables to handle user events, but we also can use them to synchronize data processing events. Isn’t it beautiful, when you have API that is simple and powerful enough to be useful in many situations? You may have started to discover that Rx.js is powerful not only because it provides us with Observables and operators but, also, because it give us very nice language to speak about asynchronous events. It has nice answers for most of your asynchronous problems.
